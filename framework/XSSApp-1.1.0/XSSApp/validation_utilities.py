...
def generate_regex_validator(regex: re.Pattern, name: str) -> Callable:
    return regex_validator(lambda element: re.match(regex, element) is not None, name)


def generate_or_regex_validator(regexes: List[re.Pattern], name: str) -> Callable:
    return regex_validator(lambda element: any(re.match(regex, element) for regex in regexes), name)
